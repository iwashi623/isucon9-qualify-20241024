...
func getUserID(r *http.Request) (int64, int, string) {
	...
	// 型アサーションを行い、型が一致しない場合のエラーハンドリング
	id, ok := userID.(int64)
	if !ok {
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	simpleUsers, err := getSimpleUesrsByIDs(dbx, userIDs)
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		return
	...
	for _, item := range items {
		seller, ok := simpleUsers[item.SellerID]
		if !ok {
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			"SELECT * FROM `items` WHERE `status` IN (?,?) AND category_id IN (?) AND `created_at` < ? ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			time.Unix(createdAt, 0),
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	simpleSellers, err := getSimpleUesrsByIDs(tx, sellerIDs)
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		tx.Rollback()
	...
	simpleBuyers, err := getSimpleUesrsByIDs(tx, buyerIDs)
	if err != nil {
		log.Print(err)
		outputErrorMsg(w, http.StatusInternalServerError, "db error")
		tx.Rollback()
	...
	for _, item := range items {
		seller, ok := simpleSellers[item.SellerID]
		if !ok {
			outputErrorMsg(w, http.StatusNotFound, "seller not found")
			tx.Rollback()
			return
		...
		if item.BuyerID != 0 {
			buyer, ok := simpleBuyers[item.BuyerID]
			if !ok {
...
func postBuy(w http.ResponseWriter, r *http.Request) {
	...
	err = tx.Get(&targetItem,
		`SELECT i.*, 
	            ...
	            u.hashed_password AS "user.hashed_password", 
	            u.address AS "user.address", 
	            u.num_sell_items AS "user.num_sell_items", 
	            u.last_bump AS "user.last_bump", 
